...
The main and the unique approach to configure penpot application is
using environement variables. All penpot related variables starts with
`PENPOT_` prefix.

**NOTE**: All the examples that comes with values, they represent the
**default** values and the examples that does not have values are
optional.
...
Penpot comes with facilities for fast creation of demo users without
the need of registration process. The demo users by default has an
expiration of 7 days and once expired, they are completly deleted with
all the generated content. Very useful for testing or demostration
purposes.
...
(LDAP). This is the example configuration we use internally for tesing
...
By default, when no SMTP (email) is configured, the email will be
printed to the console, which means that the emails will be shown in
the stdout. If you have an SMTP service, uncomment the appropiate
settings section in docker-compose.yml and configure those environment
variables.
...
If you are using the official docker compose file, this comes already
configured.


### HTTP

Allows setup the port where the backend http server will listen for requests.

```bash
PENPOT_HTTP_SERVER_PORT=6060
```

Additionally, you probably will need to setup the `PENPOT_PUBLIC_URI`
...
This is a more advanced setting, because it allows setup a port when
when a server REPL will listen. Server REPL is very usefull for
perform diagnosis, execute code on the running process, and/or perform
hotfixes of isolated pure functions without restart the process.
...
configuration options and are located in the `<dist>/js/config.js`
file.
...
automatically generates the `config.js` from that env variables.

**NOTE**: probably all frontend related configuration variables are
explained in the [Common](#common) section, this section explains
